 ...
 *   - verified but never started a membership application (Pending, no membership), or
 *   - chose a membership but never paid (Pending, unpaid), on a shorter clock.
 *
 ...
 * who has paid is never touched.
 *
 *   php artisan members:cleanup-stale-signups --dry-run
 *   php artisan members:cleanup-stale-signups --months=6 --grace-days=14 --unpaid-days=30 --sleep=2
 */
class CleanupStaleSignups extends Command
{
    protected $signature = 'members:cleanup-stale-signups
        {--months= : How many months of inactivity before an unverified/unstarted signup counts as stale (default: config).}
        {--grace-days= : Days to wait after nudging before archiving (default: config).}
        {--unpaid-days= : Days a chosen-but-unpaid membership can sit before it is nudged (default: config).}
    ...
    public function handle(StaleSignupProcessor $processor): int
    {
        ...
        $unpaidDays = $this->option('unpaid-days') !== null ? max(1, (int) $this->option('unpaid-days')) : null;
